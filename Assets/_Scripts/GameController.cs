...
public class GameController : MonoBehaviour
{
    ...
    {
        int flippedRow = Board.height - 1 - row; //Flips the row index, so that row 0 is at the bottom of the board and row 5 is at the top
    ...
    void SpawnPiece(int row, int column, int player)
    {
        ...
        //Debug.Log("Target row: " + row + ", column: " + column + ", target position: " + targetPos); 
        Vector3 spawnPos = targetPos + new Vector3(0.5f, 5f, 0); //Spawns the piece 5 units above the target position
        ...
        //Debug.Log("Player clicked at coordinates: " + Mouse.current.position.ReadValue());
        //Debug.Log($"Spawned piece for player {player} at column {column}, target Y: {targetPos.y}");
    }
    ...
    void Update()
    {
        if (currentPlayer == 1)
        {
            ...
            if (board.CheckWin(1) && !gameOver) //Prevents the lose text from showing if the player wins on the same turn as the AI, which can happen if the player wins by placing a piece that also gives the AI a winning move
            {
                //Debug.Log("AI wins!");
                loseText.SetActive(true);
                resetButton.SetActive(true);
                //enabled = false; //Stops the game, as the AI won :(
                gameOver = true;
            }
            currentPlayer = 2;
        }
        else if (Mouse.current.leftButton.wasPressedThisFrame) //Player's turn
        {
            ...
            int column = Mathf.FloorToInt((mousePos.x - boardOrigin.x) / cellSize); //Converts mouse X position to column index
            if (column >= 0 && column < Board.width && !board.IsColumnFull(column))
            {
                ...
                if (board.CheckWin(2) && !gameOver)
                {
                    ...
                    resetButton.SetActive(true);
                    //enabled = false; //Stops the game, as the player won :)
                    gameOver = true;
                }
                currentPlayer = 1;
            }
